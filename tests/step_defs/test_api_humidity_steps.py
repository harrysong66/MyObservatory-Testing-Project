...
def today_is_weekday():
    ...
    Returns:
        Boolean indicating if today is a weekday
    """
...
def request_nine_day_forecast(api_client, context):
    """Request 9-day weather forecast from API.
...
def request_invalid_endpoint(api_client):
    """Request data from invalid endpoint to test error handling.
...
def api_returns_success(context):
    """Verify API returned successful response.
...
def response_contains_forecast_data(context):
    """Verify response contains forecast data.
...
def forecast_includes_weather_forecast_array(context):
    """Verify forecast includes weatherForecast array.
...
def should_extract_humidity_day_after_tomorrow(api_client, context):
    """Extract and verify humidity for day after tomorrow.
...
def humidity_in_format(context, format_pattern):
    """Verify humidity is in specified format.
...
def humidity_contains_min_max(api_client, context):
    """Verify humidity contains both minimum and maximum values.
...
def min_humidity_lte_max(api_client, context):
    """Verify minimum humidity is less than or equal to maximum.
...
def humidity_values_are_valid_percentages(api_client, context):
    """Verify humidity values are valid percentages.
...
def extract_humidity_for_offset(api_client, context, day_offset):
    """Extract humidity for specific day offset.
    
    Args:
        api_client: WeatherAPIClient fixture
        context: Test context
        day_offset: Day offset integer
    """
...
def response_contains_fields(context, datatable):
    """Verify response contains specified fields.
    
    Args:
        context: Test context
        datatable: pytest-bdd datatable with field names
    """
...
def api_handles_error(context):
    """Verify API client handled error gracefully.
...
def returns_none_without_crash(context):
    """Verify function returned None without crashing.
    
    Args:
        context: Test context
    """
    # If we reached here, the test didn't crash
...
def humidity_matches_pattern(context, pattern):
    """Verify humidity matches regex pattern.
...
def can_parse_humidity_range(api_client, context):
    """Verify humidity range can be parsed.
...
def humidity_data_available(context):
    """Verify humidity data is available.
...
def humidity_in_valid_range(api_client, context, range_pattern):
    ...
    Args:
        api_client: WeatherAPIClient fixture
        context: Test context
        range_pattern: Range pattern (e.g., "0-100%")
    """
